...
CMDs[#CMDs + 1] = {NAME = 'guard / protect [player] [distance]', DESC = 'Protects a player by flinging others in a radius.'}
CMDs[#CMDs + 1] = {NAME = 'unguard / unprotect', DESC = 'Stops protecting a player.'}
...
CMDs[#CMDs + 1] = {NAME = 'ringparts / rp [radius] [height] [speed] [strength]', DESC = 'Creates a tornado of unanchored parts around you.'}
CMDs[#CMDs + 1] = {NAME = 'unringparts / unrp', DESC = 'Stops the part tornado.'}
...
addcmd('loopoof',{},function(args, speaker)
    oofing = true
    local oofSoundId = "rbxassetid://212222358" -- Classic "oof" sound
    local oofSounds = {}

    local function setupPlayer(player)
        if player.Character then
            local head = player.Character:WaitForChild("Head", 5)
            if head and not oofSounds[player] then
                local sound = Instance.new("Sound")
                sound.SoundId = oofSoundId
                sound.Parent = head
                oofSounds[player] = sound
            end
        end
    end
    ...
    local playerAddedConn = Players.PlayerAdded:Connect(function(player)
        player.CharacterAdded:Connect(function() setupPlayer(player) end)
        setupPlayer(player)
    end)

    local playerRemovingConn = Players.PlayerRemoving:Connect(function(player)
        if oofSounds[player] then
            oofSounds[player]:Destroy()
            oofSounds[player] = nil
        end
    end)

    repeat wait(0.5) -- Increased delay to prevent sound from being too spammy
        for player, sound in pairs(oofSounds) do
            if player and player.Character and sound and sound.Parent then
                sound:Play()
            elseif oofSounds[player] then
                oofSounds[player]:Destroy()
                oofSounds[player] = nil
            end
        end
    ...
    for _, sound in pairs(oofSounds) do if sound then sound:Destroy() end end
end)
...
gotopartDelay = 0.1
addcmd('gotopart',{'topart', 'tppart'},function(args, speaker)
	for i,v in pairs(workspace:GetDescendants()) do
		if v.Name:lower() == getstring(1):lower() and v:IsA("BasePart") then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
				speaker.Character:FindFirstChildOfClass('Humanoid').Sit = false
...
local guardLoop = nil -- Combined loop for following and guarding
...
addcmd('guard', {'protect'}, function(args, speaker)
    ...
    guarding = true
    notify("Guard", "Now guarding player " .. targetPlayer.Name)
    execCmd('noclip', {'nonotify'})

    local bambam = Instance.new("BodyAngularVelocity")
    bambam.Name = "GuardSpin"
    bambam.MaxTorque = Vector3.new(0, math.huge, 0)
    bambam.P = 50000

    -- Main loop for both following and flinging
    guardLoop = RunService.Heartbeat:Connect(function() -- Main loop for both following and flinging
        pcall(function()
            -- Exit condition check
            if not guarding or not targetPlayer or not targetPlayer.Parent or not targetPlayer.Character or not getRoot(targetPlayer.Character) or not speaker.Character or not getRoot(speaker.Character) then
                execCmd("unguard", {"nonotify"})
                return
            end

            local guardHumanoid = speaker.Character:FindFirstChildOfClass('Humanoid')
            if guardHumanoid and guardHumanoid.SeatPart then
                guardHumanoid.Sit = false
            end

            local guardRoot = getRoot(speaker.Character)
            if not guardRoot then return end

            local guardedRoot = getRoot(targetPlayer.Character)
            local guardedPosition = guardedRoot.Position
            
            local closestIntruder = nil
            local minDistance = radius + 1

            -- Find the closest intruder
            for _, player in ipairs(Players:GetPlayers()) do
                if player == targetPlayer or player == speaker then continue end

                if player.Character and getRoot(player.Character) then
                    local distance = (getRoot(player.Character).Position - guardedPosition).Magnitude
                    if distance <= radius and distance < minDistance then
                        minDistance = distance
                        closestIntruder = player
                    end
                end
            end

            local shouldSpin = (closestIntruder ~= nil)
            ...
            if shouldSpin and closestIntruder.Character and getRoot(closestIntruder.Character) then
                ...
                TweenService:Create(guardRoot, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {CFrame = targetCFrame}):Play()
            elseif not shouldSpin then
                -- Follow the guarded player.
                local targetCFrame = CFrame.new(guardedPosition - (guardedRoot.CFrame.LookVector * 5), guardedPosition)
                TweenService:Create(guardRoot, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {CFrame = targetCFrame}):Play()
            end

            -- Spinning logic
            if shouldSpin and not isSpinning then
                isSpinning = true
                pcall(function() for _, part in ipairs(speaker.Character:GetDescendants()) do if part:IsA("BasePart") then part.CustomPhysicalProperties = PhysicalProperties.new(100, 0.3, 0.5) end end bambam.Parent = getRoot(speaker.Character) bambam.AngularVelocity = Vector3.new(0, 100, 0) end)
            elseif not shouldSpin and isSpinning then
                isSpinning = false
                pcall(function() bambam.AngularVelocity = Vector3.new(0, 0, 0) bambam.Parent = nil for _, part in ipairs(speaker.Character:GetDescendants()) do if part:IsA("BasePart") and part.CustomPhysicalProperties then part.CustomPhysicalProperties = nil end end end)
            end
        end)
    end)
end)

addcmd('unguard', {'unprotect'}, function(args, speaker)
    if guarding then
        guarding = false
        isSpinning = false
        if guardLoop then guardLoop:Disconnect(); guardLoop = nil end
        ...
        pcall(function()
            if speaker.Character and getRoot(speaker.Character) then
                local root = getRoot(speaker.Character)
                if root:FindFirstChild("GuardSpin") then
                    root:FindFirstChild("GuardSpin"):Destroy()
                end
        ...
        if not (args[1] and args[1] == "nonotify") then
            notify("Guard", "Guard duty has ended.")
        end
    end
end)
...
local ringPartsLoop = nil
local ringPartsConnections = {}
local orbitingParts = {}
local originalReplicationFocus = nil

addcmd('ringparts', {'rp', 'partnado', 'parttornado'}, function(args, speaker)
	if ringPartsLoop and ringPartsLoop.Connected then
		execCmd("unringparts", { "nonotify" })
		task.wait(0.1)
	end
	...
	local strength = tonumber(args[4]) or 500

	if sethiddenproperty then
		originalReplicationFocus = speaker.ReplicationFocus
		speaker.ReplicationFocus = workspace
		sethiddenproperty(speaker, "SimulationRadius", math.huge)
	end
	...
	table.insert(ringPartsConnections, workspace.DescendantAdded:Connect(addPart))
	table.insert(ringPartsConnections, workspace.DescendantRemoving:Connect(removePart))

	notify("RingParts", "Starting part tornado...")

	local root = getRoot(speaker.Character)
	ringPartsLoop = RunService.Heartbeat:Connect(function()
		root = getRoot(speaker.Character)
		if not root then execCmd("unringparts") return end
		...
		for part, _ in pairs(orbitingParts) do
			pcall(function()
				if part and part.Parent then
					...
					local angle = math.atan2(pos.z - center.z, pos.x - center.x) + math.rad(speed)
					local targetPos = Vector3.new(center.x + radius * math.cos(angle), center.y + math.sin(os.clock() * speed + pos.x) * height, center.z + radius * math.sin(angle))
					part.Velocity = (targetPos - pos).Unit * strength
				else
					removePart(part)
				end
			end)
...
addcmd('unringparts', {'unrp'}, function(args, speaker)
	if not ringPartsLoop then
		if not (args[1] and args[1] == "nonotify") then notify("RingParts", "Not running.") end
		return
	end

	ringPartsLoop:Disconnect()
	ringPartsLoop = nil

	for _, conn in ipairs(ringPartsConnections) do conn:Disconnect() end
	ringPartsConnections = {}

	if sethiddenproperty and originalReplicationFocus then
		speaker.ReplicationFocus = originalReplicationFocus
		sethiddenproperty(speaker, "SimulationRadius", 800)
		originalReplicationFocus = nil
	end
	...
	if not (args[1] and args[1] == "nonotify") then notify("RingParts", "Stopped.") end
end)
