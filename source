...
addcmd('freecamgoto',{'fcgoto','freecamtp','fctp'},
	...
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		StartFreecam(Players[v].Character.HumanoidRootPart.CFrame)
	end
end)
...
addcmd('region',{},
	...
	if gethidden then
		...
		for i,v in pairs(players) do
				local p = Players[v]
				local region = gethidden(p, "CountryRegionCodeReplicate")
				if countries[region] then
					region = countries[region]
				end
			table.insert(regions,p.Name.."'s region is: "..region)
		end
