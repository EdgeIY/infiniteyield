...
IYConnections = {}
function addconn(signal,func)
	local conn = signal:Connect(func)
	IYConnections[#IYConnections+1] = conn
	return conn
end
...
addconn(IYMouse.KeyDown,function(Key)
	if (Key==prefix) then
...
addconn(Players.PlayerRemoving,function(player)
...
CameraChanged = addconn(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"),UpdateToViewport)
...
function updateCamera(child, parent)
	if parent ~= workspace then
		...
		CameraChanged = addconn(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"),UpdateToViewport)
		CamMoved = addconn(workspace.CurrentCamera.AncestryChanged,updateCamera)
	end
end
CamMoved = addconn(workspace.CurrentCamera.AncestryChanged,updateCamera)
...
CMDs[#CMDs + 1] = {NAME = 'unloadiy / unload', DESC = 'Unloads Infinite Yield'}
...
Clip = true
spDelay = 0.1
addconn(Players.LocalPlayer.CharacterAdded,function()
	NOFLY()
	floating = false
...
addconn(UserInputService.TextBoxFocused,function(obj)
...
addconn(Players.LocalPlayer.Chatted,function()
	wait()
...
addconn(UserInputService.InputBegan,onInputBegan)
addconn(UserInputService.InputEnded,onInputEnded)
...
addconn(IYMouse.Button1Down,function()
	for i,v in pairs(binds) do
...
addconn(Players.LocalPlayer.OnTeleport,function(State)
...
addcmd('unloadiy',{'unload','killiy'},function(args, speaker)
	lastBreakTime = tick()
	for i = #IYConnections,1,-1 do
		pcall(function() IYConnections[i]:Disconnect() end)
		IYConnections[i] = nil
	end
	if PARENT == COREGUI:FindFirstChild('RobloxGui') then
		for _,v in pairs({ScaledHolder,selectionBox,selected,QuickCapture}) do
...
fcRunning = false
local Camera = workspace.CurrentCamera
addconn(workspace:GetPropertyChangedSignal("CurrentCamera"),function()
...
RolewatchConnection = addconn(Players.PlayerAdded,function(player)
...
addconn(Players.PlayerAdded,function(plr)
...
if not isLegacyChat then
	addconn(TextChatService.MessageReceived,function(message)
...
addconn(IYMouse.Move,checkTT)

addconn(CaptureService.CaptureBegan,function()
	PARENT.Enabled = false
end)

addconn(CaptureService.CaptureEnded,function()
